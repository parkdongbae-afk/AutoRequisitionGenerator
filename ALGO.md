...
- **`verifyCount: { "sel": "span[class^=count_goods--]", "regex": "(\\d+)" }`** (v1.8.0): **부분저장 감지**. Chrome 저장 스냅샷에서 체크된 일부 상품 행이 누락되는 실측(네이버: 화면 체크 6건 → 파일 4행, 하단 플로팅 패널 `count_goods` "6개의 상품"·GNB "담긴상품6" 존재 / G마켓: 하단 계산기 `div.cart_total` 첫 `benefit_item` "상품수 7개" → 파일 체크 스탬프 4행, 상품금액 142,730−할인 4,430 = 138,300 = 파일 4행+누락 3행으로 금액 교차검증 일치). 엔진이 `sel` 텍스트에서 총수 N을 읽고 추출 수 M보다 크면 `countMismatch: {expected: N, actual: M}` 반환(0건 추출 시엔 기존 거부 팝업이 담당하므로 검증 생략) → docstore가 doc에 저장·summarize 노출 → store.addDocs에서 경고 팝업("체크 N개 중 캡처에 M개만 저장…스크롤 후 재저장 또는 품의캡처 이용"). 파일에 없는 상품은 복구 불가하므로 **감지·안내**가 목적. naver-cart·gmarket-cart 등록(티처몰은 카운터 없어 미적용)

...
async function captureLiveHtml() {
  ...
  try {
    // 화면을 순간적으로 25%로 축소하면 가상화 목록이 뷰포트 안에 모든 행을 렌더한다.
    // 적용 후 300ms 대기해 리렌더를 기다리고 수집하며, outerHTML까지 축소 상태에서 찍은 뒤
    // 사용자의 원래 화면 비율로 즉시 되돌린다(되돌리는 모습은 화면에 보이지 않는다).
    // 캡처 본문에는 축소 흔적(html 태그의 zoom 인라인)을 남기지 않는다.
    const de = document.documentElement
    let prevZoom = ''
    try { prevZoom = de.style.zoom || ''; de.style.zoom = '0.25' } catch (e) {}
    await sleep(300)
    collectFrom(store)
    ...
    stampAll()
    // 축소 상태에서 저장 (사용자 요구: 300ms 대기 후 축소 상태로 mhtml 저장)
    capturedHtml = de.outerHTML
  } catch (e) {}
  // 사용자 화면을 즉시 원래 배율로 복원 — 직후 캡처이므로 축소 과정은 보이지 않는다
  try { document.documentElement.style.zoom = '' } catch (e) {}
  let html = capturedHtml || document.documentElement.outerHTML
  // 캡처 본문에서 축소 흔적(html 태그의 zoom 인라인) 제거 — 뷰어는 원래 크기로 표시
  html = html.replace(/(<html\b[^>]*?)\s+style="[^"]*"/i, '$1')
